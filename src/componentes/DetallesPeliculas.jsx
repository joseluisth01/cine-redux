...
const DetallesPeliculas = () => {
    ...
    const [entradasCompradas, setEntradasCompradas] = useState(0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setPelicula(data);
                // Obtener el estado de favorito desde el almacenamiento local
                const favoritoLocalStorage = localStorage.getItem('favorito-' + id);
                if (favoritoLocalStorage === 'true') {
                    setFavorito(true);
                }
            } catch (error) {
    ...
    const handleReservar = (event) => {
        ...
        localStorage.setItem('reserva-nombre', nombre);
        localStorage.setItem('reserva-email', email);
        localStorage.setItem('reserva-cantidad', cantidad);
    ...
    const handleFavoritoClick = () => {
        ...
        // Guardar el estado de favorito en localStorage
        localStorage.setItem('favorito-' + id, nuevoEstadoFavorito ? 'true' : 'false');
    };
            ...
            <div className="detalles" style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <div className="informacion">
                    ...
                    <div className='info2'>
                        ...
                        <a href="#openModal2" className='buttoncargar2'>FAVORITOS
                            <span role="img" aria-label="favorito" onClick={handleFavoritoClick}>
                                {favorito ? "⭐" : "☆"}
                            </span></a>
                    </div>
